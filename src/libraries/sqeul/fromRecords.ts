 ...
 */
type RecordsObject<T extends RecordType> = {
  /**
   ...
   */
  filter: (predicate: (record: T) => boolean) => RecordsObject<Partial<T>>
  /**
...
export function fromRecords<T extends RecordType>(records: T[]): RecordsObject<T> {
  return {
    ...
    fullJoin: <U extends RecordType>(otherRecords: U[], on: keyof T & keyof U) => {
      ...
      return fromRecords(leftResult.concat(rightResult.filter((row) => !leftResult.some((r) => r[on as any] as any === row[on as any])))) as unknown as RecordsObject<Partial<T> & Partial<U>>
    },
